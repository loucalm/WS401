<template>
  <div class="h-screen overflow-hidden bg-white text-black">
              <main class="mx-auto flex h-full w-full max-w-105 flex-col bg-white">
                <section :class="headerClass" class="rounded-b-3xl px-4 pb-6 pt-4 sm:px-5 sm:pb-7 sm:pt-5">
                  <div class="relative flex items-center justify-center">
                    ...
                    <h1 class="px-12 text-center font-ui text-[38px] leading-none text-black sm:text-[46px] md:text-[58px]">Add activity</h1>
                  </div>
                </section>

                <section v-if="currentStep === 1" class="flex-1 overflow-y-auto px-4 pt-6 sm:px-5 sm:pt-7">
                  <div class="mx-auto flex w-full max-w-[430px] items-center gap-2.5 px-2 sm:gap-3 sm:px-4">
                    <div class="relative flex h-16 w-16 items-center justify-center rounded-full border-8 border-main bg-white text-main sm:h-20 sm:w-20 sm:border-10">
                      <span class="font-ui text-4xl font-bold leading-none sm:text-6xl">1</span>
                    </div>
                    <div class="h-2 flex-1 rounded-full bg-main sm:h-2.5"></div>
                    <div class="h-9 w-9 rounded-full border-6 border-main bg-white sm:h-11 sm:w-11 sm:border-[9px]"></div>
                  </div>

                  <h2 class="mt-6 text-center font-ui text-[36px] leading-[0.98] text-black sm:mt-7 sm:text-[46px] md:text-[58px]">Type of activity</h2>

                  <p v-if="loading" class="mt-6 text-center text-body-16 text-grey sm:mt-7">Chargement des activites...</p>
                  ...
                  <div v-else class="mt-6 space-y-4 sm:mt-8 sm:space-y-5">
                    ...
                    >
                      <div class="flex h-18 w-18 shrink-0 items-center justify-center rounded-2xl sm:h-22 sm:w-22" :class="activity.iconBoxClass">
                        <component :is="activity.icon" class="h-10 w-10 sm:h-13 sm:w-13" :class="activity.iconColorClass" />
                      </div>

                      <span class="font-ui text-[28px] leading-none text-black sm:text-[42px] md:text-[58px]">{{ activity.label }}</span>
                    </button>
                  </div>
                </section>

                <section v-else-if="currentStep === 2 && selectedCategory === 'travel'" class="flex-1 px-4 pb-30 pt-3 sm:px-5 sm:pt-5">
                  <div class="mx-auto flex w-full max-w-[700px] items-center px-0">
                    <div class="h-9 w-9 rounded-full bg-main sm:h-12 sm:w-12"></div>
                    <div class="h-2 flex-1 bg-main sm:h-2.5"></div>
                    <div class="relative flex h-18 w-18 shrink-0 items-center justify-center rounded-full border-7 border-main bg-white text-main sm:h-22 sm:w-22 sm:border-8">
                      <span class="absolute inset-0 flex items-center justify-center font-ui text-title-h2 font-bold leading-none sm:text-[52px]">2</span>
                    </div>
                    <div class="h-2 flex-1 bg-main sm:h-2.5"></div>
                    <div class="h-9 w-9 rounded-full border-5 border-main bg-white sm:h-12 sm:w-12 sm:border-6"></div>
                  </div>

                  <h2 class="mt-5 text-center font-ui text-[34px] leading-none text-black sm:text-[44px]">{{ stepTitle }}</h2>
                  ...
                  <div class="relative mt-7">
                    ...
                    >
                      <span class="font-ui text-[26px] leading-none text-black sm:text-[34px]">{{ transportMode }}</span>
                      <ChevronDownIcon class="h-7 w-7 text-black transition-transform sm:h-9 sm:w-9" :class="isTransportMenuOpen ? 'rotate-180' : ''" />
                    </button>
                    ...
                    >
                      ...
                      >
                        <span class="font-ui text-[22px] leading-none text-black sm:text-[28px]">{{ option }}</span>
                        <span v-if="transportMode === option" class="font-ui text-[18px] leading-none text-main">OK</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div class="mt-8 flex gap-3 overflow-x-auto pb-2">
                    <button
                      ...
                      :class="selectedFuel === fuel ? 'border-2 border-main bg-white text-main' : 'bg-main-light text-main'"
                  ...
                  <h3 class="mt-8 font-ui text-[28px] leading-none text-black sm:text-[34px]">Distance</h3>

                  <div class="mt-8 flex justify-center">
                    <div class="inline-flex min-w-[148px] items-center justify-center rounded-[999px] bg-white px-8 py-9 shadow-[0_10px_18px_rgba(0,0,0,0.18)] sm:min-w-[180px] sm:px-10 sm:py-11">
                      <span class="font-ui text-[36px] leading-none text-black sm:text-[46px]">{{ distance }} KM</span>
                    </div>
                  </div>

                  <div class="mt-10 flex items-center justify-between gap-3 px-1 sm:mt-14 sm:gap-4 sm:px-5">
                    <button type="button" class="flex h-14 min-w-[74px] items-center justify-center rounded-3xl bg-main-light px-4 font-ui text-[30px] leading-none text-main sm:h-18 sm:min-w-[98px] sm:px-6 sm:text-[42px]" @click="adjustDistance(-5)">-5</button>
                    <button type="button" class="flex h-14 w-14 items-center justify-center rounded-full bg-main-light font-ui text-[30px] leading-none text-main sm:h-18 sm:w-18 sm:text-[42px]" @click="adjustDistance(-1)">-</button>
                    <button type="button" class="flex h-14 w-14 items-center justify-center rounded-full bg-main-light font-ui text-[30px] leading-none text-main sm:h-18 sm:w-18 sm:text-[42px]" @click="adjustDistance(1)">+</button>
                    <button type="button" class="flex h-14 min-w-[74px] items-center justify-center rounded-3xl bg-main-light px-4 font-ui text-[30px] leading-none text-main sm:h-18 sm:min-w-28 sm:px-6 sm:text-[42px]" @click="adjustDistance(5)">+5</button>
                  </div>
                </section>

                <section v-else-if="currentStep === 2 && selectedCategory === 'food'" class="flex-1 overflow-y-auto px-4 pb-30 pt-3 sm:px-5 sm:pt-5">
                  <div class="mx-auto flex w-full max-w-[700px] items-center px-0">
                    <div class="h-9 w-9 rounded-full bg-tertiary sm:h-12 sm:w-12"></div>
                    <div class="h-2 flex-1 bg-tertiary sm:h-2.5"></div>
                    <div class="relative flex h-18 w-18 shrink-0 items-center justify-center rounded-full border-7 border-tertiary bg-white text-tertiary sm:h-22 sm:w-22 sm:border-8">
                      <span class="absolute inset-0 flex items-center justify-center font-ui text-title-h2 font-bold leading-none sm:text-[52px]">2</span>
                    </div>
                    <div class="h-2 flex-1 bg-tertiary sm:h-2.5"></div>
                    <div class="h-9 w-9 rounded-full border-5 border-tertiary bg-white sm:h-12 sm:w-12 sm:border-6"></div>
                  </div>

                  <h2 class="mt-5 text-center font-ui text-[34px] leading-none text-black sm:text-[44px]">Food</h2>

                  <h3 class="mt-8 font-ui text-[28px] leading-none text-black sm:text-[34px]">Type of consumption</h3>

                  <div class="mt-5 grid grid-cols-3 gap-4">
                    <button v-for="option in foodConsumptionOptions" :key="option.id" type="button" class="text-left" @click="selectedFoodConsumption = option.id">
                      <div class="flex aspect-square items-center justify-center rounded-[20px] border-2 bg-white" :class="selectedFoodConsumption === option.id ? 'border-secondary bg-white' : option.cardClass">
                        <component :is="option.icon" class="h-20 w-20" />
                      </div>
                    </button>
                  </div>

                  <p class="mt-5 font-ui text-[30px] leading-none text-black sm:text-[38px]">{{ selectedFoodConsumptionLabel }}</p>

                  <div class="mt-8 space-y-7 pb-4">
                    <div v-for="section in foodSections" :key="section.id">
                      <p class="font-ui text-[24px] leading-none text-black sm:text-[28px]">{{ section.label }} :</p>
                      <div class="mt-4 grid grid-cols-3 gap-4">
                        <button v-for="option in section.options" :key="option.id" type="button" class="text-center" @click="selectFoodOption(section.id, option.id)">
                          <div class="flex h-26 items-center justify-center rounded-[18px] border-2 bg-secondary-light" :class="selectedFoodOptions[section.id] === option.id ? 'border-secondary' : 'border-transparent'">
                            <component :is="option.icon" class="h-13 w-13 text-secondary" />
                          </div>
                          <p class="mt-2 font-ui text-body-16 leading-[1.05] text-black">{{ option.label }}</p>
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                <section v-else-if="currentStep === 2 && selectedCategory === 'consumption'" class="flex-1 overflow-y-auto px-4 pb-30 pt-3 sm:px-5 sm:pt-5">
                  <div class="mx-auto flex w-full max-w-[700px] items-center px-0">
                    <div class="h-9 w-9 rounded-full bg-secondary sm:h-12 sm:w-12"></div>
                    <div class="h-2 flex-1 bg-secondary sm:h-2.5"></div>
                    <div class="relative flex h-18 w-18 shrink-0 items-center justify-center rounded-full border-7 border-secondary bg-white text-secondary sm:h-22 sm:w-22 sm:border-8">
                      <span class="absolute inset-0 flex items-center justify-center font-ui text-title-h2 font-bold leading-none sm:text-[52px]">2</span>
                    </div>
                    <div class="h-2 flex-1 bg-secondary sm:h-2.5"></div>
                    <div class="h-9 w-9 rounded-full border-5 border-secondary bg-white sm:h-12 sm:w-12 sm:border-6"></div>
                  </div>

                  <h2 class="mt-5 text-center font-ui text-[34px] leading-none text-black sm:text-[44px]">Consumption</h2>
                  ...
                  <div class="relative mt-7">
                    ...
                    >
                      <span class="font-ui text-[26px] leading-none text-black sm:text-[34px]">{{ consumptionSource }}</span>
                      <ChevronDownIcon class="h-7 w-7 text-black transition-transform sm:h-9 sm:w-9" :class="isConsumptionMenuOpen ? 'rotate-180' : ''" />
                    </button>
                    ...
                    >
                      ...
                      >
                        <span class="font-ui text-[22px] leading-none text-black sm:text-[28px]">{{ option }}</span>
                        <span v-if="consumptionSource === option" class="font-ui text-[18px] leading-none text-secondary">OK</span>
                      </button>
                    </div>
                  </div>

                  <h3 class="mt-8 font-ui text-[28px] leading-none text-black sm:text-[34px]">Surface</h3>

                  <div class="mt-8 flex justify-center">
                    <div class="inline-flex min-w-[210px] items-center justify-center rounded-[999px] bg-white px-10 py-9 shadow-[0_10px_18px_rgba(0,0,0,0.18)] sm:min-w-[250px] sm:px-12 sm:py-10">
                      <span class="font-ui text-[36px] leading-none text-black sm:text-[46px]">{{ surface }} M²</span>
                    </div>
                  </div>

                  <div class="mt-10 flex items-center justify-between gap-3 px-1 sm:mt-12 sm:gap-4 sm:px-5">
                    <button type="button" class="flex h-14 min-w-[74px] items-center justify-center rounded-3xl bg-secondary-light px-4 font-ui text-[30px] leading-none text-secondary sm:h-18 sm:min-w-[98px] sm:px-6 sm:text-[42px]" @click="adjustSurface(-10)">-10</button>
                    <button type="button" class="flex h-14 w-14 items-center justify-center rounded-full bg-secondary-light font-ui text-[30px] leading-none text-secondary sm:h-18 sm:w-18 sm:text-[42px]" @click="adjustSurface(-1)">-</button>
                    <button type="button" class="flex h-14 w-14 items-center justify-center rounded-full bg-secondary-light font-ui text-[30px] leading-none text-secondary sm:h-18 sm:w-18 sm:text-[42px]" @click="adjustSurface(1)">+</button>
                    <button type="button" class="flex h-14 min-w-[74px] items-center justify-center rounded-3xl bg-secondary-light px-4 font-ui text-[30px] leading-none text-secondary sm:h-18 sm:min-w-28 sm:px-6 sm:text-[42px]" @click="adjustSurface(10)">+10</button>
                  </div>

                  <h3 class="mt-9 font-ui text-[28px] leading-none text-black sm:mt-12 sm:text-[34px]">Duration</h3>
                  ...
                  <div class="mt-6">
                    ...
                    <div class="mt-2 flex items-center justify-between">
                      <span v-for="mark in durationMarks" :key="mark" class="font-ui text-[18px] leading-none text-black sm:text-[22px]">{{ mark }}h</span>
                    </div>
                  </div>
                </section>

                <section v-else-if="currentStep === 2 && selectedCategory === 'clothing'" class="flex-1 overflow-y-auto px-4 pb-30 pt-3 sm:px-5 sm:pt-5">
                  <div class="mx-auto flex w-full max-w-[700px] items-center px-0">
                    <div class="h-9 w-9 rounded-full bg-cloth sm:h-12 sm:w-12"></div>
                    <div class="h-2 flex-1 bg-cloth sm:h-2.5"></div>
                    <div class="relative flex h-18 w-18 shrink-0 items-center justify-center rounded-full border-7 border-cloth bg-white text-cloth sm:h-22 sm:w-22 sm:border-8">
                      <span class="absolute inset-0 flex items-center justify-center font-ui text-title-h2 font-bold leading-none sm:text-[52px]">2</span>
                    </div>
                    <div class="h-2 flex-1 bg-cloth sm:h-2.5"></div>
                    <div class="h-9 w-9 rounded-full border-5 border-cloth bg-white sm:h-12 sm:w-12 sm:border-6"></div>
                  </div>

                  <h2 class="mt-5 text-center font-ui text-[34px] leading-none text-black sm:text-[44px]">Clothing</h2>

                  <h3 class="mt-8 font-ui text-[28px] leading-none text-black sm:text-[34px]">Purchasing type :</h3>

                  <div class="mt-5 grid grid-cols-3 gap-4">
                    <button v-for="option in clothingPurchaseOptions" :key="option.id" type="button" class="text-center" @click="selectedClothingPurchase = option.id">
                      <div class="flex aspect-square items-center justify-center rounded-[20px] border-2 bg-cloth-light" :class="selectedClothingPurchase === option.id ? 'border-cloth bg-white' : 'border-transparent'">
                        <component :is="option.icon" class="h-18 w-18 text-cloth sm:h-20 sm:w-20" />
                      </div>
                      <p class="mt-2 font-ui text-body-16 leading-[1.05] text-black">{{ option.label }}</p>
                    </button>
                  </div>

                  <p class="mt-5 font-ui text-[30px] leading-none text-black sm:text-[38px]">{{ selectedClothingPurchaseLabel }}</p>

                  <div class="mt-8 space-y-7 pb-4">
                    <div v-for="section in clothingSections" :key="section.id">
                      <p class="font-ui text-[24px] leading-none text-black sm:text-[28px]">{{ section.label }} :</p>
                      <div class="mt-4 grid grid-cols-3 gap-4">
                        <button v-for="option in section.options" :key="option.id" type="button" class="text-center" @click="selectClothingOption(section.id, option.id)">
                          <div class="flex h-26 items-center justify-center rounded-[18px] border-2 bg-cloth-light" :class="selectedClothingOptions[section.id] === option.id ? 'border-cloth bg-white' : 'border-transparent'">
                ...
                <section v-else class="flex flex-1 flex-col px-4 pb-30 pt-3 sm:px-5 sm:pt-5">
                  <div class="mx-auto flex w-full max-w-[520px] items-center px-0">
                    <div :class="step3GaugeFillClass" class="h-9 w-9 rounded-full sm:h-12 sm:w-12"></div>
                    <div :class="step3GaugeFillClass" class="h-2 flex-1 sm:h-2.5"></div>
                    <div :class="step3GaugeRingClass" class="relative flex h-18 w-18 shrink-0 items-center justify-center rounded-full border-7 bg-white sm:h-22 sm:w-22 sm:border-8">
                      <span :class="step3TextAccentClass" class="absolute inset-0 flex items-center justify-center font-ui text-title-h2 font-bold leading-none sm:text-[52px]">3</span>
                    </div>
                  </div>

                  <h2 class="mt-7 text-center font-ui text-[34px] leading-none text-black sm:text-[44px]">Recap : {{ stepTitle }}</h2>

                  <div v-if="selectedCategory === 'food'" class="mt-12 space-y-8 sm:space-y-10">
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Type : {{ selectedFoodConsumptionLabel }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Base : {{ getFoodLabel('base') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Protein : {{ getFoodLabel('protein') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Vitamines : {{ getFoodLabel('vitamines') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Bonus : {{ getFoodLabel('bonus') }}</p>
                  </div>

                  <div v-else-if="selectedCategory === 'consumption'" class="mt-16 space-y-14 sm:space-y-18">
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Type : {{ consumptionSource }}</p>
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Surface : {{ surface }} m²</p>
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Duration : {{ duration }}h</p>
                  </div>

                  <div v-else-if="selectedCategory === 'clothing'" class="mt-12 space-y-8 sm:space-y-10">
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Purchase : {{ selectedClothingPurchaseLabel }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Top outfits : {{ getClothingLabel('top') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Stockings : {{ getClothingLabel('bottom') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Underwear : {{ getClothingLabel('underwear') }}</p>
                    <p class="font-ui text-[25px] leading-none text-black sm:text-[34px]">Accessory : {{ getClothingLabel('accessory') }}</p>
                  </div>

                  <div v-else class="mt-16 space-y-16 sm:space-y-20">
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Transport : {{ transportMode }}</p>
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Fuel : {{ selectedFuel }}</p>
                    <p class="font-ui text-[32px] leading-none text-black sm:text-[44px]">Distance : {{ distance }}km</p>
                  </div>

                  <div class="mt-auto pb-4 text-center">
                    <p :class="step3TextAccentClass" class="font-ui text-[44px] font-bold leading-none sm:text-[64px]">{{ formattedCo2 }} KG CO2E</p>
                  </div>
                </section>

                <div v-if="currentStep > 1" class="pointer-events-none fixed inset-x-0 bottom-0 z-20 px-4 pb-4 sm:px-5 sm:pb-5">
                  <div class="mx-auto grid w-full max-w-105 grid-cols-2 gap-4 bg-white pt-3 pointer-events-auto sm:gap-5">
                    <button type="button" :class="actionBackClass" @click="goToStep(currentStep === 2 ? 1 : 2)">Back</button>
                    <button type="button" :class="actionNextClass" @click="currentStep === 2 ? goToStep(3) : confirmWizard()">{{ currentStep === 2 ? 'Next' : 'Confirm' }}</button>
                  </div>
                </div>
              </main>
            </div>
          ...
          TravelIcon.template = `
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <path d="M2.8 19V8.5h6V5h5.2v4H21V19" />
              <path d="M7 11h1.2" />
              <path d="M7 14h1.2" />
              <path d="M10.8 7h1.2" />
              <rect x="9" y="12" width="11" height="4.2" rx="1.6" />
              <circle cx="11.8" cy="17.4" r="1.2" fill="currentColor" stroke="none" />
              <circle cx="17.2" cy="17.4" r="1.2" fill="currentColor" stroke="none" />
            </svg>
          `;
          ...
          FoodIcon.template = `
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M4.2 16.5h9.5v2.3H4.2z" />
              <path d="M4.2 12.9h9.5v2.3H4.2z" />
              <path d="M4.2 9.3h9.5v2.3H4.2z" />
              <path d="M5 8.4c.9-2.6 3.3-4.2 6.5-4.2s5.6 1.7 6.5 4.2H5z" />
              <path d="M16.5 3.7h4.8l-1.2 14.8h-3.3z" />
            </svg>
          `;
          ...
          ConsumptionIcon.template = `
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M3.5 10.2 12 4l8.5 6.2v9.3a1 1 0 0 1-1 1H4.5a1 1 0 0 1-1-1v-9.3z" />
              <path d="M10 13.5h4V20h-4z" fill="white" opacity="0.85" />
            </svg>
          `;
          ...
          ClothingIcon.template = `
            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M8.1 4.2 12 6.6l3.9-2.4 5.1 3.5-2.1 4.1-2.3-.8V20a1 1 0 0 1-1 1H8.4a1 1 0 0 1-1-1v-9l-2.3.8L3 7.7l5.1-3.5z" />
            </svg>
          `;
          ...
          export default {
            setup() {
              ...
              const foodConsumptionOptions = [
                { id: "meal", icon: FoodMealIcon, cardClass: "border-transparent bg-main-light" },
                { id: "fresh", icon: FoodFreshPlateIcon, cardClass: "border-transparent bg-secondary-light" },
                { id: "dish", icon: FoodDishIcon, cardClass: "border-transparent bg-tertiary-light" },
              ];
              ...
              const foodSections = [
                ...
                },
                {
                  ...
                  options: [
                    ...
                    { id: "cooked-vegetables", label: "Cooked vegetables", icon: CookedVegetablesIcon },
                  ],
                },
              ];
              ...
              const normalizeToCategory = (name = "") => {
                ...
                if (value.includes("travel") || value.includes("transport")) return "travel";
                if (value.includes("food") || value.includes("meal") || value.includes("alimentation")) return "food";
                if (value.includes("consumption") || value.includes("housing") || value.includes("home")) return "consumption";
                if (value.includes("cloth") || value.includes("fashion") || value.includes("vetement")) return "clothing";

                return "travel";
              };

              const shownTypes = computed(() => {
                const source = activityTypes.value.length > 0 ? activityTypes.value : fallbackTypes;
              ...
              const selectedCategory = computed(() => normalizeToCategory(selectedType.value?.name || "travel"));
              ...
              const selectedClothingPurchaseLabel = computed(() => {
                const option = clothingPurchaseOptions.find((item) => item.id === selectedClothingPurchase.value);
              ...
              const co2Value = computed(() => {
                ...
                if (selectedCategory.value === "consumption") {
                  const factor = consumptionFactors[consumptionSource.value] || consumptionFactors.Electricity;
                  return surface.value * duration.value * factor;
                }

                if (selectedCategory.value === "clothing") {
                  const purchaseFactor = clothingPurchaseFactors[selectedClothingPurchase.value] || clothingPurchaseFactors["second-hand"];
                  const selectionTotal = Object.values(selectedClothingOptions.value).reduce((total, itemId) => total + (clothingItemFactors[itemId] || 0), 0);
                  return selectionTotal * purchaseFactor;
                }

                const transportFactors = co2Factors[transportMode.value] || co2Factors.Car;
                const factor = transportFactors[selectedFuel.value] || transportFactors.Diesel;
                return distance.value * factor;
              });
              const formattedCo2 = computed(() => co2Value.value.toFixed(2));
              const headerClass = computed(() => {
                if (currentStep.value > 1 && selectedCategory.value === "food") return "bg-tertiary-light";
                if (currentStep.value > 1 && selectedCategory.value === "consumption") return "bg-secondary-light";
                if (currentStep.value > 1 && selectedCategory.value === "clothing") return "bg-cloth-light";
              ...
              const step3GaugeFillClass = computed(() => (selectedCategory.value === "food" ? "bg-tertiary" : selectedCategory.value === "consumption" ? "bg-secondary" : selectedCategory.value === "clothing" ? "bg-cloth" : "bg-main"));
              const step3GaugeRingClass = computed(() => (selectedCategory.value === "food" ? "border-tertiary" : selectedCategory.value === "consumption" ? "border-secondary" : selectedCategory.value === "clothing" ? "border-cloth" : "border-main"));
              const step3TextAccentClass = computed(() => (selectedCategory.value === "food" ? "text-tertiary" : selectedCategory.value === "consumption" ? "text-secondary" : selectedCategory.value === "clothing" ? "text-cloth" : "text-main"));
              ...
              const getClothingLabel = (sectionId) => {
                ...
                const option = section?.options.find((item) => item.id === selectedClothingOptions.value[sectionId]);
                return option?.label || "-";
              };
              ...
              const getFoodLabel = (sectionId) => {
                ...
                const option = section?.options.find((item) => item.id === selectedFoodOptions.value[sectionId]);
                return option?.label || "-";
              };
